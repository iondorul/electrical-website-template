...
(function () {
  function template() {
    return `
      <h5 class="settings-section-title">Cont & Securitate</h5>
      <p class="settings-section-desc">
        Datele contului tău și opțiunile de securitate.
      </p>

      <form id="accountProfileForm">
        <div class="row g-3">
          <div class="col-12 col-md-6">
            <label for="accFullName" class="form-label fw-semibold">Nume utilizator</label>
            <input type="text" class="form-control" id="accFullName" required>
          </div>
          <div class="col-12 col-md-6">
            <label for="accEmail" class="form-label fw-semibold">Email</label>
            <input type="email" class="form-control" id="accEmail" readonly>
          </div>
          <div class="col-12 col-md-6">
            <label for="accPhone" class="form-label fw-semibold">Telefon</label>
        ...
        <div class="mt-3">
          <button type="submit" class="btn btn-primary fw-semibold px-4" id="btnSaveAccountProfile">
            <i class="fas fa-floppy-disk me-2"></i> Salvează modificările
          </button>
        </div>
      </form>

      <div class="settings-subblock">
        <div class="settings-subblock-title">Schimbă parola</div>
        <form id="changePasswordForm">
          <div class="row g-3">
            <div class="col-12 col-md-4">
              <label for="accCurrentPassword" class="form-label">Parola curentă</label>
              <div class="position-relative settings-password-field">
                ...
                <button type="button" class="settings-password-toggle" data-target="accCurrentPassword" tabindex="-1" aria-label="Arată/ascunde parola">
                  <i class="fas fa-eye"></i>
                </button>
              </div>
            </div>
            <div class="col-12 col-md-4">
              <label for="accNewPassword" class="form-label">Parolă nouă</label>
              <div class="position-relative settings-password-field">
                <input type="password" class="form-control pe-5" id="accNewPassword" autocomplete="new-password">
                <button type="button" class="settings-password-toggle" data-target="accNewPassword" tabindex="-1" aria-label="Arată/ascunde parola">
                  <i class="fas fa-eye"></i>
                </button>
              </div>
            </div>
            <div class="col-12 col-md-4">
              <label for="accConfirmPassword" class="form-label">Confirmă parola nouă</label>
              <div class="position-relative settings-password-field">
                <input type="password" class="form-control pe-5" id="accConfirmPassword" autocomplete="new-password">
                <button type="button" class="settings-password-toggle" data-target="accConfirmPassword" tabindex="-1" aria-label="Arată/ascunde parola">
                  <i class="fas fa-eye"></i>
                </button>
              </div>
        ...
        <div class="d-flex align-items-center justify-content-between">
          <div>
            <div class="settings-subblock-title mb-1">
              ...
              <span class="settings-badge-soon ms-2">ÎN CURÂND</span>
            </div>
            <p class="text-muted small mb-0" style="max-width: 420px;">
              Adaugă un nivel suplimentar de securitate contului tău la autentificare.
            </p>
          </div>
  ...
  async function saveProfile(container, btn) {
    ...
    if (!fullName) {
      Toast.show("Numele utilizatorului este obligatoriu.", "danger");
      return;
    }
    ...
    try {
      ...
      if (response && response.success) {
        Toast.show("Profilul a fost actualizat cu succes.", "success");
      } else {
        Toast.show(
          (response && response.message) || "Nu s-a putut actualiza profilul.",
          "danger",
        );
      }
    } catch (err) {
  ...
  async function changePassword(container, btn) {
    ...
    if (!current || !next || !confirm) {
      Toast.show("Completează toate câmpurile pentru schimbarea parolei.", "danger");
      return;
    }
    if (next.length < 6) {
      Toast.show("Noua parolă trebuie să aibă cel puțin 6 caractere.", "danger");
      return;
    }
    if (next !== confirm) {
      Toast.show("Parola nouă și confirmarea nu coincid.", "danger");
      return;
    }
    ...
    try {
      ...
      if (response && response.success) {
        Toast.show("Parola a fost schimbată cu succes.", "success");
        ...
        Toast.show(
          (response && response.message) || "Nu s-a putut schimba parola.",
          "danger",
        );
      }
    } catch (err) {
      console.error("Eroare la schimbarea parolei:", err);
      Toast.show(err.message || "Eroare de rețea la schimbarea parolei.", "danger");
    } finally {
      btn.disabled = false;
      btn.innerHTML = originalHtml;
    }
