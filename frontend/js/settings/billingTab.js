...
(function () {
  function templateLoading() {
    return `
      <h5 class="settings-section-title">${t("settings.billing.title", "Abonament & Plăți")}</h5>
      <p class="settings-section-desc">
        ${t("settings.billing.desc", "Detalii despre planul curent și facturarea abonamentului.")}
      </p>
    ...
    return `
      ...
      <div class="card border-0" style="background: #f8f9fb; border-radius: 12px;">
        <div class="card-body p-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
          <div>
            <span class="badge bg-secondary mb-2">${t("settings.billing.freePlan", "Plan Free")}</span>
            <div class="fw-semibold">${t("settings.billing.freeAccess", "Ai acces la funcțiile de bază ElectricalVPF ERP.")}</div>
            <div class="text-muted small">${t("settings.billing.freeNoExpiry", "Fără costuri, fără dată de expirare.")}</div>
          </div>
          <a href="erp-plans.html" class="btn btn-primary fw-semibold px-4" id="btnUpgradePlan">
            <i class="fas fa-arrow-up-right-dots me-2"></i> ${t("settings.billing.upgrade", "Upgrade la Pro")}
          </a>
        </div>
      </div>

      <div class="settings-subblock">
        <div class="settings-subblock-title">${t("settings.billing.invoiceHistory", "Istoric Facturi Abonament")}</div>
        <div class="settings-placeholder-box" id="billingInvoiceHistory">
          <i class="fas fa-file-invoice fs-4 mb-2 d-block"></i>
          ${t("settings.billing.noInvoicesYet", "Nu există facturi de abonament emise încă.")}<br>
          ${t("settings.billing.historyWillAppear", "Istoricul plăților va apărea aici.")}
        </div>
      </div>
    `;
  }
        ...
        <div class="card-body p-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
          ...
          <div>
            <a href="erp-plans.html" class="plan-badge plan-badge-pro mb-2" style="display: flex; margin: 0; width: 210px; box-sizing: border-box; border-radius: 8px;"><i
                class="fas fa-crown plan-badge-icon"></i><span>${t("settings.billing.proPlan", "Plan Pro")}</span></a>
            <div class="fw-semibold" id="billingRenewalDate">${t("settings.billing.renewalLoading", "Data reînnoirii: se încarcă...")}</div>
            <div class="text-muted small">${t("settings.billing.autoRenewMonthly", "Abonamentul se reînnoiește automat lunar.")}</div>
            <a href="erp-plans.html" class="billing-manage-link mt-2"><i class="fas fa-credit-card me-2"></i>${t("settings.billing.manage", "Administrează abonamentul")}</a>
          </div>
        </div>
      </div>
          ...
          historyEl.outerHTML = invoices
            .map((inv) => {
              const dateLabel = inv.date
                ? new Date(inv.date).toLocaleDateString(getCurrentLocaleCode(), {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })
                : t("settings.billing.dateUnavailable", "Dată indisponibilă");
              const amountLabel = `${(inv.amount / 100).toFixed(2)} ${String(
                inv.currency || "",
              ).toUpperCase()}`;
              const isPaid = inv.status === "paid";
              const statusLabel = isPaid ? t("settings.billing.paid", "Plătită") : inv.status;
              ...
              const pdfLink = pdfUrl
                ? `<a href="${pdfUrl}" target="_blank" rel="noopener" class="billing-invoice-pdf-link ms-3" title="${t("settings.billing.openInvoice", "Deschide factura")}"><i class="fas fa-file-pdf"></i></a>`
                : "";
              return `
        ...
        if (renewalDateEl) {
          try {
            ...
            if (subResponse && subResponse.status === "error") {
              renewalDateEl.textContent = t(
                "settings.billing.renewalCheckFailed",
              ...
              if (currentPeriodEnd) {
                ...
                renewalDateEl.textContent = t(
                  ...
                  "Abonamentul se reînnoiește pe {{date}}",
                  { date: formatted },
                );
              } else {
                renewalDateEl.textContent = t(
                  "settings.billing.renewalUnavailable",
                  "Data reînnoirii: indisponibilă momentan",
                );
              }
            }
            ...
            if (bannerEl && paymentFailedAt) {
              bannerEl.innerHTML = `
                <div class="alert alert-warning d-flex align-items-center gap-2 py-2 px-3 mb-0 mt-3" role="alert">
                  ...
                  <span>${t("settings.billing.paymentFailedWarning", "Ultima plată a eșuat. Stripe va reîncerca automat plata în zilele următoare — nu trebuie să faci nimic acum.")}</span>
                </div>
              `;
            }
          } catch (err) {
