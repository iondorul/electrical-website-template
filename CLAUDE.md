...
- **`invoices`, `invoice_items`, `payments`, `materials` nu aveau niciun `CREATE TABLE` în repo** — rezolvat prin migrarea `013_recover_missing_tables.sql` (20 august 2026), care le recreează exact (inclusiv enum-ul `invoice_status`), reprodusă din `pg_dump --schema-only` pe DB-ul local. Coloanele/indexul adăugate ulterior de `003`/`019` (`sent_at`, `sent_to_email`, `min_stock`, `idx_payments_invoice_id`) au fost mutate direct în `013`, iar `003`/`019` au fost editate să nu mai presupună că tabelele există deja (altfel ar eșua pe o instanță nouă, unde rulează înaintea lui `013`).
...
- **Rezolvat** (28 august 2026, pre-deploy hardening): numerotarea migrărilor avea duplicate — câte două fișiere `004_*` și `005_*` (`004_reports_prerequisites.sql`/`004_user_profile_fields.sql`, `005_generated_reports.sql`/`005_user_trial_started_at.sql`). Fișierele fără nicio dependență de ordine (`004_reports_prerequisites.sql` — deja doar un comentariu, logica reală mutată în `013`; `005_generated_reports.sql` — `CREATE TABLE` independent, fără nicio migrare ulterioară care să-l altereze) au fost redenumite (`git mv`, istoricul git păstrat) în `019_reports_prerequisites.sql`/`020_generated_reports.sql`, primele numere libere. `004_user_profile_fields.sql`/`005_user_trial_started_at.sql` au rămas neschimbate — a doua depinde de ordine (trebuie să ruleze înaintea lui `011_drop_trial_started_at.sql`, care îi șterge coloana). Nu există niciun tabel/mecanism de urmărire a migrărilor deja rulate (fără `schema_migrations`, fără runner) — se aplică manual, în ordinea numerică a fișierelor, deci redenumirea nu risca nicio re-rulare; singurul risc real era ordinea, verificată explicit mai sus.
...
- **CORS restricționat** (28 august 2026, pre-deploy hardening): `server.js` verifică explicit originea cererii într-o listă albă — `FRONTEND_URL` (normalizat prin `new URL(...).origin`, ca să ignore un eventual path din valoare, ex. `/frontend`) plus `http://localhost:5500`/`http://127.0.0.1:5500` mereu permise pentru development local, indiferent de `FRONTEND_URL`. `credentials: true` NU e activat — autentificarea e 100% prin header `Authorization: Bearer <jwt>` (`middleware/authMiddleware.js`), niciun cookie de sesiune nicăieri în backend/frontend, deci nu e nevoie de el.
