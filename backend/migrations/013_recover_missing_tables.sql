...
CREATE INDEX idx_invoices_client_id ON public.invoices USING btree (client_id) WHERE (is_active = true);

CREATE INDEX idx_invoices_project_id ON public.invoices USING btree (project_id) WHERE (is_active = true);

CREATE INDEX idx_materials_category ON public.materials USING btree (category) WHERE (is_active = true);

CREATE INDEX idx_materials_created_by ON public.materials USING btree (created_by) WHERE (is_active = true);

CREATE INDEX idx_payments_invoice_id ON public.payments USING btree (invoice_id);

CREATE UNIQUE INDEX uq_invoices_quote_id ON public.invoices USING btree (quote_id) WHERE ((is_active = true) AND (quote_id IS NOT NULL));

CREATE UNIQUE INDEX uq_user_invoice_number ON public.invoices USING btree (created_by, invoice_number) WHERE (is_active = true);

CREATE UNIQUE INDEX uq_user_item_code ON public.materials USING btree (created_by, item_code) WHERE ((is_active = true) AND (item_code IS NOT NULL));

--
-- Triggers (funcția public.update_updated_at_column() e deja creată în schema.sql)
--

CREATE TRIGGER trg_update_invoices_updated_at BEFORE UPDATE ON public.invoices FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER trg_update_materials_updated_at BEFORE UPDATE ON public.materials FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
